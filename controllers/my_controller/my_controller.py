...
def grabBall():
  global LM, RM, finger1, leftFrontMotor, confirmGreen, colorVal, finger2, arm2, leftBackMotor, color, rightFrontMotor, leftColor, leftGreen, rightBackMotor, arm3, arm4, frontColor, rightColor, rightGreen, leftDist, frontDist, temp
...
# Describe this function...
def setMotor(LM, RM):
  global finger1, leftFrontMotor, confirmGreen, colorVal, finger2, arm2, leftBackMotor, color, rightFrontMotor, leftColor, leftGreen, rightBackMotor, arm3, arm4, frontColor, rightColor, rightGreen, leftDist, frontDist, temp
...
def setColor():
  ...
  rightGreen = color[0 + 1] / color[-1 + 1]
  if color[0 + 1] > 145:
...
arm4 = myRobot.getDevice("arm4")
encObj[arm4] = arm4.getPositionSensor()
arm4.setPosition(float("inf"))
arm4.setVelocity(0)
encObj[arm4].enable(timeStep)
encCount[arm4] = 0
lastEncReset[encObj[arm4]] = 0

finger1 = myRobot.getDevice("finger1")
encObj[finger1] = finger1.getPositionSensor()
finger1.setPosition(float("inf"))
finger1.setVelocity(0)
encObj[finger1].enable(timeStep)
encCount[finger1] = 0
lastEncReset[encObj[finger1]] = 0

finger2 = myRobot.getDevice("finger2")
encObj[finger2] = finger2.getPositionSensor()
finger2.setPosition(float("inf"))
finger2.setVelocity(0)
encObj[finger2].enable(timeStep)
encCount[finger2] = 0
lastEncReset[encObj[finger2]] = 0
...
initTime = myRobot.getTime()
while myRobot.step(timeStep) != -1:
  if (myRobot.getTime() - initTime) * 1000.0 > 1700:
    break
arm2.setVelocity((0 / 100.0) * arm2.getMaxVelocity())
grabBall()
